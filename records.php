            ...
            <div id="records" class="flex flex-wrap">
                ...
                <?php for ($x = 0; $x < count($records); $x++) : ?>
                    <div class="md:w-1/2 lg:w-1/3 w-full p-2">
                        <div class="record group w-full h-[150px] duration-200 rounded-md shadow-inner  cursor-pointer bg-slate-100/10 flex flex-nowrap justify-center items-center gap-2 text-slate-100 font-bold hover:bg-slate-100 hover:shadow-md active:bg-slate-100/70" data-recordDetails='<?= json_encode($records[$x]) ?>'>
                            <?php for ($i = 0; $i < 5; $i++) : ?>
                                <span class="py-6 min-w-[60px] text-center text-2xl text-gray-700 bg-slate-200 rounded-md shadow-md block group-hover:text-white group-hover:bg-[#ae94ce] duration-300"><?= $records[$x]["luck_num"][$i] ?></span>
                            <?php endfor; ?>
                        </div>
                    </div>
